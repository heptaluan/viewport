...
import './FourBenignList.scss'
import { useHistory } from 'react-router-dom'
import { Table, Select, Button, Space, message, Input } from 'antd'
import { getFourBenignList } from '../../../api/api'
import MenuList from '../../../components/MenuList/MenuList'
import HeaderList from '../../../components/HeaderList/HeaderList'

const FourBenignList = () => {
  ...
  const initPagination = result => {
    let page = localStorage.getItem('FourBenignList') ? JSON.parse(localStorage.getItem('FourBenignList')) : ''
  ...
  const fetchFourBenignList = async () => {
    const result = await getFourBenignList(isFinish, searchId)
  ...
  const onPageChange = e => {
    ...
    localStorage.setItem('FourBenignList', JSON.stringify(newPagination))
    setPagination(newPagination)
  }
  ...
  const handleShowDoctorDetail = record => {
    ...
    localStorage.setItem('FourBenignList', JSON.stringify(pagination))
    history.push(`/fourViewer?id=${record.id}&from=${history.location.pathname}`)
  }
  ...
  return (
    <div className="study-list-box">
      ...
      <div className="study-list-container-wrap">
        <MenuList defaultSelectedKeys={'4-0'} userInfo={userInfo} />
