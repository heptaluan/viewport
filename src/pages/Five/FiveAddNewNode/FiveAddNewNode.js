...
import './FiveAddNewNode.scss'
import { Radio, Select, Input, Button } from 'antd'
import cornerstone from 'cornerstone-core'

const { Option } = Select

const FiveAddNewNode = props => {
      ...
      {props.toolList?.map((item, index) => (
        <div className="add-box" key={item.uuid}>
          ...
          <div className="list">
            ...
            <Button
              type={item.isBenign === 1 ? 'primary' : null}
              style={{ marginRight: '15px' }}
              size="small"
              onClick={e => props.updateIsBenign(1, item.uuid)}
            >
              否
            </Button>
            <Button
              type={item.isBenign === 0 || item.isBenign > 1 ? 'primary' : null}
              size="small"
              onClick={e => props.updateIsBenign(0, item.uuid)}
            >
              是
            </Button>
          </div>
          {item.isBenign !== 1 ? (
            <div className="list">
              ...
              <Button
                type={item.isBenign === 2 ? 'primary' : null}
                style={{ marginRight: '15px' }}
                size="small"
                onClick={e => props.updateIsBenign(2, item.uuid)}
              >
                良
              </Button>
              <Button
                type={item.isBenign === 3 ? 'primary' : null}
                size="small"
                onClick={e => props.updateIsBenign(3, item.uuid)}
              >
                恶
              </Button>
            </div>
