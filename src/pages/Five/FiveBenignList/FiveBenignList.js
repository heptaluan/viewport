...
import './FiveBenignList.scss'
import { useHistory } from 'react-router-dom'
import { Table, Select, Button, Space, message, Input } from 'antd'
import { getFiveBenignList } from '../../../api/api'
import MenuList from '../../../components/MenuList/MenuList'
import HeaderList from '../../../components/HeaderList/HeaderList'

const FiveBenignList = () => {
  ...
  const initPagination = result => {
    let page = localStorage.getItem('FiveBenignList') ? JSON.parse(localStorage.getItem('FiveBenignList')) : ''
  ...
  // 获取总医生列表数据
  const fetchFiveBenignList = async () => {
    const result = await getFiveBenignList(isFinish, searchId)
  ...
  const onPageChange = e => {
    ...
    localStorage.setItem('FiveBenignList', JSON.stringify(newPagination))
    setPagination(newPagination)
  }
  ...
  const handleShowDoctorDetail = record => {
    ...
    history.push(`/fiveViewer?id=${record.id}&from=${history.location.pathname}`)
  }
  ...
  return (
    <div className="study-list-box">
      ...
      <div className="study-list-container-wrap">
        <MenuList defaultSelectedKeys={'5-0'} userInfo={userInfo} />
