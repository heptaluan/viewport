...
const SecondBenignList = () => {
  ...
  const initPagination = result => {
    let page = localStorage.getItem('SecondBenignList') ? JSON.parse(localStorage.getItem('SecondBenignList')) : ''
    const newPagination = Object.assign({}, pagination)
    if (page !== '') {
  ...
  const handleShowDetail = record => {
    ...
    localStorage.setItem('SecondBenignList', JSON.stringify(newPagination))
    history.push(
      `/secondViewer?id=${record.id}&kyPrimaryId=${record.kyPrimaryId}&isFinish=${record.isFinish}&type=1&from=${history.location.pathname}`
    )
  }
  ...
  return (
    <div className="study-list-box">
      ...
      <div className="study-list-container-wrap">
        <MenuList defaultSelectedKeys={'2-4'} userInfo={userInfo} />
...
export default SecondBenignList
