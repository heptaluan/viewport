...
const SixViewer = () => {
  ...
  //       fetcImagehData(result.data.data.info.pcode)
  ...
  //   const fetcImagehData = async pcode => {
  //     if (!pcode) return
  //     const res = await getFourViewerImageList(pcode)
  ...
  useEffect(() => {
    ...
    const fetcImagehData = async url => {
      if (!url) return
      const res = await getFourViewerImageList(url)
      const newList = res.data.list
      const imageList = []
      for (let i = 0; i < newList.length; i++) {
    ...
    if (data.code === 10000) {
      ...
      for (let i = 0; i < res.length; i++) {
        nodulesList.push({
          ...
          diameterSize:
            resultInfo[i] && resultInfo[i].newDiameter ? formatDiameter(resultInfo[i].newDiameter) : formatDiameter(res[i].diameter),
  ...
  const formatSecondprimaryNodeData = data => {
    ...
    for (let i = 0; i < data.nodeInfo.length; i++) {
      ...
      nodulesList.push({
        ...
        type: resultInfo && resultInfo.featuresType ? resultInfo.featuresType : res.featuresType,
  ...
  const updateNewNoduleList = checkState => {
    const checkItme = noduleList.find(item => item.checked === true)
    checkItme.review = true
    checkItme.remark = checkState
    setNoduleList([...noduleList])
    if (params.type === 'mission') {
      saveSecondprimaryResults(checkItme)
    } else {
